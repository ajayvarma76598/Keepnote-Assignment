...
//		modelMap.addAttribute("newNote", new Note());
	...
	public String delUser(@RequestParam("noteId") int noteId) {
	...
	public String updateNote(@RequestParam("noteId") int noteId,
			@RequestParam("noteTitle") String noteTitle,
			@RequestParam("noteContent") String noteContent,
			@RequestParam("noteStatus") String noteStatus) {
		if(noteDao.getNoteById(noteId)!=null) {
			noteDao.getNoteById(noteId).setNoteTitle(noteTitle);
			noteDao.getNoteById(noteId).setNoteContent(noteContent);
			noteDao.getNoteById(noteId).setNoteStatus(noteStatus);
			noteDao.UpdateNote(noteDao.getNoteById(noteId));
			return "redirect:/";
		}
		return "redirect:/";
	}
